  ...
  return (
    <div className={`mobile-controls${enabled ? '' : ' is-disabled'}`} aria-hidden={!enabled}>
      <div className="mobile-controls__pad-cluster">
        ...
        <button type="button" className={`mobile-controls__sprint${sprintLocked ? ' is-active' : ''}`} aria-pressed={sprintLocked} aria-label={t('controls.sprintLock')} disabled={travelMode === 'bicycle'} onClick={() => commands.press('toggleSprint')}>{t('controls.sprintLock')}</button>
      </div>
      ...
      <div className="mobile-controls__actions" role="group" aria-label={t('controls.move')}>
        <button type="button" className="mobile-controls__action mobile-controls__action--jump" aria-label={t('controls.jump')} disabled={!enabled || travelMode === 'bicycle'} onPointerDown={jump} onClick={(event) => { if (event.detail === 0) commands.press('jump'); }}>{t('controls.jump')}</button>
        {travelMode === 'bicycle' && <button type="button" className="mobile-controls__action mobile-controls__action--brake" aria-label={t('controls.brake')} onPointerDown={brakeStart} onPointerUp={brakeEnd} onPointerCancel={brakeEnd} onLostPointerCapture={brakeEnd} onBlur={()=>brakeEnd()} onKeyDown={(event) => { if ((event.key === ' ' || event.key === 'Enter') && !event.repeat) { event.preventDefault(); commands.setBrake(true); } }} onKeyUp={(event) => { if (event.key === ' ' || event.key === 'Enter') { event.preventDefault(); commands.setBrake(false); } }}>{t('controls.brake')}</button>}
        <button type="button" className="mobile-controls__action mobile-controls__action--cycle" aria-label={travelMode === 'bicycle' ? t('controls.dismount') : t('controls.mount')} disabled={!enabled || !canInteract} onClick={() => commands.press('interact')}>{travelMode === 'bicycle' ? t('controls.dismount') : t('controls.mount')}</button>
      </div>
    </div>
  );
}
