...
export function CharacterScreen({ initialProfile, initialCharacterId, signedIn, reducedMotion, onSubmit, onBack }: { initialProfile?: ExplorerProfile; initialCharacterId: string; signedIn: boolean; reducedMotion: boolean; onSubmit: (profile: ExplorerProfile) => void; onBack: () => void }) {
  const t = useT();
  ...
  return <ShellFrame title={t('shell.character.title')} hints={t('shell.hints.back')} onBack={onBack}>
    <div className="shell-store">
      ...
      <div><ProfileForm onSubmit={onSubmit} onPreview={setPreview} initialProfile={initialProfile} initialCharacterId={initialCharacterId} signedIn={signedIn}/></div>
    </div>
  </ShellFrame>;
}
