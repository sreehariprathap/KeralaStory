...
import { STORE_ITEMS, GUEST_LOADOUT, isUnlocked, storeItems, resolveEquipped, applyEquippedCharacter, PROCEDURAL_CHARACTER_ID } from '../src/content/store/catalog';
...
describe('store catalogue', () => {
  ...
  it('resolves defaults when nothing is equipped', () => {
    expect(resolveEquipped(undefined, true)).toEqual({ carId: 'admin', carColor: '#b3121f', bikeId: 'roadster', characterId: 'procedural' });
  });

  it('keeps valid equipped ids and falls back per field on unknown ones', () => {
    expect(resolveEquipped({ carId: 'supercar', carColor: '#16181c', bikeId: 'yamaha', characterId: 'straw-hat' }, true))
      .toEqual({ carId: 'supercar', carColor: '#16181c', bikeId: 'yamaha', characterId: 'straw-hat' });
    expect(resolveEquipped({ carId: 'mazda-rx7', carColor: 'purple', bikeId: 'nope', characterId: 'tommy' }, true))
      .toEqual({ carId: 'admin', carColor: '#b3121f', bikeId: 'roadster', characterId: 'procedural' });
  });

  it('lets a guest use only the launch skin, one car and one bicycle', () => {
    expect(GUEST_LOADOUT).toEqual({ car: 'admin', bike: 'roadster', character: 'procedural' });
    for (const item of STORE_ITEMS) {
      expect(isUnlocked(item.kind, item.id, true)).toBe(true);
      expect(isUnlocked(item.kind, item.id, false)).toBe(item.id === GUEST_LOADOUT[item.kind]);
    }
  });

  it('falls back to the guest loadout for a signed-out player, keeping the paint', () => {
    expect(resolveEquipped({ carId: 'supercar', carColor: '#16181c', bikeId: 'yamaha', characterId: 'straw-hat' }, false))
      .toEqual({ carId: 'admin', carColor: '#16181c', bikeId: 'roadster', characterId: 'procedural' });
  });
