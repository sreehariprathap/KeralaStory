...
import { LocaleSchema, SaveV2Schema, PreferencesSchema, DEFAULT_SETTINGS } from '../src/contracts';
it('validates locale and standalone preferences', () => {
  expect(LocaleSchema.parse('ml')).toBe('ml');
  expect(() => LocaleSchema.parse('xx')).toThrow();
  expect(PreferencesSchema.parse({})).toEqual({locale:'en', controls:'auto', haptics:true});
});
